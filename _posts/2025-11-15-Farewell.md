...
tags: [xss,js,bruteforce,python,waf,obfuscation]
...
To escalate the XSS vuln, we can add the `onerror` attribute to the `img` element, however this request was flagged by the [WAF](#waf) and the response returned the same page as above. After some modifications of the initial payload you can find a workable payload. Below my process of testing is shown.
